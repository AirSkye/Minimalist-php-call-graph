...
func method_def_analysis(path string, basepath string) {
    ...
    defer func() {
        ...
        if r := recover(); r != nil {
            // 记录错误信息
            fmt.Printf("在 method_def_analysis 中捕获到 panic，错误信息：%v\n", r)
            // 在这里可以添加额外的日志记录或错误处理逻辑
            log.Println("发生错误：", r)
        }
    }()
...
func preprocessFile(path string, basepath string) {
    ...
    defer func() {
        ...
        if r := recover(); r != nil {
            // 记录错误信息
            fmt.Printf("在 preprocessFile 中捕获到 panic，错误信息：%v\n", r)
            // 在这里可以添加额外的日志记录或错误处理逻辑
            log.Println("发生错误：", r)
        }
    }()
...
func processFile(path string, basepath string, tx *db.Tx) {
    ...
    defer func() {
        ...
        if r := recover(); r != nil {
            // 记录错误信息
            fmt.Printf("在 processFile 中捕获到 panic，错误信息：%v\n", r)
            // 在这里可以添加额外的日志记录或错误处理逻辑
            log.Println("发生错误：", r)
        }
    }()
